...
class MathUtilTest(unittest.TestCase):
	...
	def test_nearestPowerOf2(self):
		...
		self.assertTrue(np.array(2) == mu.nearestPowerOf2(np.array(0.1)))
		...
		self.assertTrue(np.all(np.array([2]) == mu.nearestPowerOf2(np.array([0.1]))))
		self.assertTrue(np.all(np.array([0, 2, 2, 4, 4, 8, 8, 16]) == mu.nearestPowerOf2(np.array([0, 0.1, 2, 2.1, 4, 4.1, 8, 8.1]))))
